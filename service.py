...
def process_page(page_nr, srch_param_name, srch_str, file_orig_path):
    ...
    subs = []
    descriptions = []
    for counter, match in enumerate(SUBTITLE_RE.finditer(content.decode('utf-8', 'ignore'))):
...
def action_download(subdivx_id, workdir):
    ...
    if html_content is None:
        log("No content found in selected subtitle intermediate detail/final download page",
            level=LOGFATAL)
        return []
    match = DETAIL_PAGE_LINK_RE.search(html_content.decode('utf-8', 'ignore'))
    if match is None:
    ...
    if html_content is None:
        ...
        return []
    match = DOWNLOAD_LINK_RE.search(html_content.decode('utf-8', 'ignore'))
    if match is None:
...
def main():
    ...
    if action in ('search', 'manualsearch'):
        ...
        debug_dump_path(xbmcvfs.translatePath(__addon__.getAddonInfo('profile')),
